...
def _get_mlx_csc_spmm_kernel():
    global _MLX_CSC_SPMM_KERNEL
    if _MLX_CSC_SPMM_KERNEL is None:
        import mlx.core as mx
        ...
        source = """
            ...
            Idx start = indptr[col];
            Idx end = indptr[col + 1];
            T acc = 0;
            for (Idx p = start; p < end; p++) {
                Idx row = indices[p];
                acc += lhs[k * M + row] * data[p];
            }
            out[k * N + col] = acc;
        """
        _MLX_CSC_SPMM_KERNEL = mx.fast.metal_kernel(
            name="dense_csc_spmm",
...
def _rmatmul_mlx(lhs: np.ndarray, mat: np.ndarray | csr_array) -> np.ndarray:
    ...
    if issparse(mat):
        ...
        csc = mat_csr.tocsc()
        mx_data = mx.array(csc.data.astype(np.float32, copy=False))
        mx_indices = mx.array(csc.indices.astype(np.int32, copy=False))
        mx_indptr = mx.array(csc.indptr.astype(np.int32, copy=False))
        mx_lhs = mx.array(lhs_2d.astype(np.float32, copy=False))

        kernel = _get_mlx_csc_spmm_kernel()
        tg_k = min(K, 16)
        tg_n = min(N, 16)
        out = kernel(
